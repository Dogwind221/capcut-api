...
- **内置剪辑思维库**：蒸馏自 B站《2026最新剪映专业版全套教程》27集（UP: PR教程周一）——
  剪辑本质=讲故事（情绪51%+事件23%+节奏10%）、需求→API 决策链 9 步表、
  音乐思维（四问题检索/开场3式/打破预期留白）、卡点思维（波形看鼓点+librosa 节拍检测）、
  转场五等级体系（青铜→王者）与 9 大高级转场→API 枚举映射、字幕行业规范、调色分级边界、
  爆款拉片方法论——把用户的自然语言剪辑需求准确转化为 API 调用序列
- **配套排障手册**：SKILL.md 收录常见踩坑（虚拟环境/FFmpeg PATH/版本兼容/草稿备份/
  PowerShell 中文 JSON 编码）与本机部署记录
